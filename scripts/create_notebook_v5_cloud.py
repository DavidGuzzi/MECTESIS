"""
Genera notebooks/experimentos_univariados_v5_cloud.ipynb
Fork de create_notebook_v4_cloud.py con:
  ...
  - Cache nuevo results/univariate_v5_vertexai/ (no toca v4)

Ejecutar: python scripts/create_notebook_v5_cloud.py
"""
...
cells.append(md(
    ...
    "Chronos-2 expone unicamente cuantiles (no samples i.i.d.). En v4_cloud se pasaban 5 "
    "cuantiles ordenados a `crps_ensemble`, lo cual rompe el supuesto i.i.d. del estimador "
    "de Hersbach e introduce un sesgo relativo positivo de hasta ~100% en el benchmark "
    "N(0,1). **v5 calcula el CRPS via la representacion integral propia** "
    "$\\\\text{CRPS}(F,y) = 2 \\\\int_0^1 QL_\\\\tau(F^{-1}(\\\\tau), y)\\\\,d\\\\tau$ "
    "aproximada con regla del trapecio sobre **K=39 cuantiles uniformes** "
    "{0.025, 0.050, ..., 0.975}. Es la misma familia que la WQL del paper de Chronos "
    "(Ansari et al. 2024); convergencia $\\\\mathcal{O}(1/K^2)$, scoring rule propia. "
    "Para los modelos clasicos se sigue usando `crps_gaussian` (exacta) o `crps_ensemble` "
    "con paths bootstrap (samples genuinos del modelo). "
    "**Los CSV en `results/univariate_v5_vertexai/` no son comparables directamente con "
    "los de v4_cloud para Chronos.**"
))
...
out_path = Path("notebooks/experimentos_univariados_v5_cloud.ipynb")
