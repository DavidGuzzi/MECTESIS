"""
Generador del notebook `experimentos_covariables_v4_cloud.ipynb`.

Replica la estructura de los notebooks v4_cloud (univariado y multivariado)
para experimentos con covariables: T adaptativo, H adaptativo, R=500, logging
dual, cache CSV, Chronos-2 instanciado una sola vez.

Cubre 18 experimentos en 6 bloques: C-A (ARIMAX variando beta/rho_x), C-B
(dinamica AR mas rica), C-C (multi-covariable), C-D (volatilidad GARCH), C-E
(VARX multivariado), C-F (cointegracion ADL-ECM).

Uso:
    python scripts/gen_notebook_covariables_v4_cloud.py
Genera:
    notebooks/experimentos_covariables_v4_cloud.ipynb
"""
import json
from pathlib import Path


NB_PATH = Path(__file__).resolve().parents[1] / "notebooks" / "experimentos_covariables_v4_cloud.ipynb"
...
# Cell 0 — title
# ────────────────────────────────────────────────────────────────────────────
TITLE = """# Experimentos con Covariables v4 Cloud (Vertex AI)
