"""Genera el cuadro resumen completo de los experimentos multivariados v6.

Fork de `build_full_univariate_summary.py` adaptado a la metrica CONJUNTA
multivariada. Lee los CSV producidos por el Monte Carlo multivariado v6
(55 experimentos M-A.1..M-G.9) en `notebooks/results/multivariate_v6_vertexai/`
y emite un unico cuadro combinado (todos los T lado a lado en una sola hoja)
mas, por paridad con el script univariado, tres fragmentos por T.

Para cada experimento se reportan los cocientes Chronos/Clasico de las dos
metricas multivariadas conjuntas que el engine almacena en la fila virtual
`var = -1`:

  * Trace MSFE  -- traza de la matriz de error cuadratico medio (puntual),
  * avgCRPS     -- promedio del CRPS marginal a traves de las k variables
                   (probabilistica).
...
es menor al 0.5%. Ambas metricas son no negativas, asi que el cociente es la
division directa Chronos/Clasico (menor = mejor).

La primera columna identifica cada experimento por la descripcion del proceso
generador (DGP), extraida de la notebook v6 (ej. "VAR(1) baja interdependencia"),
convertida a LaTeX legible. `adjustbox` ajusta el cuadro a una hoja A4.

Uso:
    python scripts/build_full_multivariate_summary.py
"""
...
RESULTS_DIR = ROOT / "notebooks" / "results" / "multivariate_v6_vertexai"
OUTPUT_DIR = ROOT / "entrega" / "tesis" / "output"
NOTEBOOK = ROOT / "notebooks" / "experimentos_multivariados_v6_cloud.ipynb"
...
# Metricas multivariadas conjuntas (fila virtual var = -1 del engine).
METRICS = ["trace_msfe", "avg_crps"]
METRIC_LABELS = {
...
def latexify_desc(s: str) -> str:
    """Convierte la descripcion cruda del DGP multivariado a LaTeX legible.

    Maneja: flechas de causalidad (Y1->Y2), simbolos griegos (Sigma, Gamma y
    minusculas seguidas de '='), exponentes L^n, s=n, k=n y subindices _n.
    Es pragmatica: las etiquetas son texto plano casi siempre seguro.
    """
    # Flechas de causalidad: '->' (con o sin espacios) -> \rightarrow
    s = s.replace("->", r"$\rightarrow$")
    # Mayusculas griegas usadas en los DGPs multivariados.
    s = re.sub(r"\bSigma\b", r"$\\Sigma$", s)
    s = re.sub(r"\bGamma(\d*)\b",
               lambda m: r"$\Gamma%s$" % (("_{%s}" % m.group(1)) if m.group(1) else ""), s)
    # Exponentes y combinaciones de operador de rezago.
...
def load_descriptions() -> dict[str, str]:
    ...
    Cada celda de experimento llama `verify_dgp_mv("M-A.1 -- descripcion", ...)`.
    Se toma el texto despues de '--'. La clave usa el punto (M-A.1), como en la
    notebook; el descubrimiento de CSV usa guion bajo (M-A_1) y se mapea aparte.
    """
    if not NOTEBOOK.exists():
        print(f"[warn] notebook no encontrada ({NOTEBOOK}); se usaran IDs como etiqueta")
        return {}
    nb = json.loads(NOTEBOOK.read_text(encoding="utf-8"))
    pat_desc = re.compile(r'verify_dgp_mv\(\s*"(M-[A-G]\.\d+)\s*--\s*([^"]+)"')
    pat_md = re.compile(r'#{2,3}\s*(M-[A-G]\.\d+)\s*[-—]+\s*(.+)')
    out: dict[str, str] = {}
...
def discover_experiments() -> list[tuple[str, int]]:
    """Devuelve [(block, idx)] ordenados por (orden de bloque, idx numerico).

    No asume que cada experimento tenga el T=50: algunos de alta dimension
    (p.ej. M-C.4/M-C.5, M-G.7) corren un subconjunto de T. Se deduplica sobre
...
def load_csv(block: str, idx: int, T: int) -> pd.DataFrame | None:
    """Carga la fila conjunta (var = -1) de un experimento, horizontes enteros."""
...
# acotado a un rango claro (misma convencion que el cuadro univariado).
...
BG_T_MAX = 0.30    # mezcla en el extremo
...
def _fmt_ratio(ratio: float) -> str:
    """Formatea el cociente: 2 decimales en el rango normal y notacion compacta
    (p. ej. '1.7e5') en los extremos, para que casos degenerados como M-B.6
    (Chronos diverge cerca de raiz unitaria) no inflen el ancho de columna y
    obliguen a `adjustbox` a encoger todo el cuadro."""
    a = abs(ratio)
...
def ratio_cell(ratio: float) -> str:
    ...
    Intensidad por fold-change |log(ratio)| (escala log, simetrica). Zona
    neutra a +-0.5% de 1. El triangulo (color pleno) marca el ganador.
    """
    if pd.isna(ratio):
...
def compute_ratio(metric: str, v_chronos: float, v_classical: float) -> float:
    """Cociente Chronos/Clasico. Trace MSFE y avgCRPS son no negativos: division
    directa (menor = mejor)."""
...
# ---------------------------------------------------------------------------
# Render del cuadro por T (un tabular por T, ajustado a una hoja vertical).
# ---------------------------------------------------------------------------
...
def build_combined_summary(
    ...
    -> metricas (Trace MSFE / avgCRPS). Una fila por experimento.
    """
...
WRAPPER_BODY = (
    r"\input{multiv_full_summary_T50.tex}" "\n\n"
    r"\clearpage" "\n"
    r"\input{multiv_full_summary_T100.tex}" "\n\n"
    r"\clearpage" "\n"
    r"\input{multiv_full_summary_T200.tex}" "\n"
)

WRAPPER_BODY_COMBINED = r"\input{multiv_full_summary_combined_table.tex}" "\n"
...
def try_compile(wrapper_path: Path) -> Path | None:
    ...
    if pdflatex is None:
        ...
        return None
    # --enable-installer: MiKTeX baja automaticamente paquetes faltantes sin prompt.
...
def main() -> None:
    ...
    for T in T_LIST:
        ...
        out_path = OUTPUT_DIR / f"multiv_full_summary_T{T}.tex"
        header = (
            f"% Resumen completo multivariado v6 (metrica conjunta), T={T}.\n"
            "% Generado automaticamente por scripts/build_full_multivariate_summary.py\n"
        )
        out_path.write_text(header + body, encoding="utf-8")
        print(f"[ok]   {out_path.relative_to(ROOT)}")

    wrapper_path = OUTPUT_DIR / "multiv_full_summary.tex"
    ...
    combined_frag = OUTPUT_DIR / "multiv_full_summary_combined_table.tex"
    combined_frag.write_text(
        "% Resumen multivariado v6 (metrica conjunta) -- vista combinada (todos los T).\n"
        "% Generado automaticamente por scripts/build_full_multivariate_summary.py\n"
        + combined_body,
        encoding="utf-8",
    )
    ...
    combined_wrapper = OUTPUT_DIR / "multiv_full_summary_combined.tex"
