"""
Generador del notebook `experimentos_covariables_v6_cloud.ipynb`.

Fork de gen_notebook_covariables_v5_cloud.py con la grilla expandida:
  - 51 experimentos en 10 bloques (C-A..C-J), vs 25 en 8 bloques de v5
  - Amplia los bloques existentes (factorial beta x rho_x en C-A, mas phi,
    alpha_ecm, delta, Phi, A/Sigma en el resto) y agrega dos bloques nuevos:
      * C-I: relacion senal/ruido (variacion de sigma_y, sigma_x)
      * C-J: interaccion dinamica AR x persistencia de la covariable (phi x rho_x)
  - Misma config global que v5 (SEED, T_LIST, H_BY_T, R_LIST) -> comparable
  - Mismo calculo de CRPS de Chronos que v5 (estimador propio via cuantiles,
    regla trapezoidal del pinball loss, K=39 cuantiles uniformes)
  - Cache nuevo results/covariate_v6_vertexai/ (no toca v5)

Uso:
    python scripts/gen_notebook_covariables_v6_cloud.py
Genera:
    notebooks/experimentos_covariables_v6_cloud.ipynb
"""
import json
from pathlib import Path


NB_PATH = Path(__file__).resolve().parents[1] / "notebooks" / "experimentos_covariables_v6_cloud.ipynb"
...
# Cell 0 — title
# ────────────────────────────────────────────────────────────────────────────
TITLE = """# Experimentos con Covariables v6 Cloud (Vertex AI)
...
R_LIST  = [500]
T_LIST  = [50, 100, 200]
RESULTS = Path("results/covariate_v6_vertexai")
...
# ════════════════════════════════════════════════════════════════════════════
# Experimentos nuevos en v6 — ampliacion de bloques (C-A.6+, C-B.3+, ...) y
# bloques nuevos C-I (senal/ruido) y C-J (interaccion phi x rho_x).
# ════════════════════════════════════════════════════════════════════════════

# ── C-A: completar la factorial 3x3 beta x rho_x (esquinas faltantes) ────────
CA += [
    (
        ...
        exp_uni_cell(
            ...
            "ARIMAX_DGP", "seed=SEED",
            "SARIMAXModel((1, 0, 0), name_suffix='con X'),\n        chronos_cov1",
            "phi=0.6, beta=0.8, sigma_y=1.0, sigma_x=1.0, rho_x=0.95",
            SARIMAX_NAME_1,
        ),
    ),
    (
        ...
        exp_uni_cell(
            ...
            "ARIMAX_DGP", "seed=SEED",
            "SARIMAXModel((1, 0, 0), name_suffix='con X'),\n        chronos_cov1",
            "phi=0.6, beta=0.8, sigma_y=1.0, sigma_x=1.0, rho_x=0.0",
            SARIMAX_NAME_1,
        ),
    ),
    (
        ...
        exp_uni_cell(
            ...
            "ARIMAX_DGP", "seed=SEED",
            "SARIMAXModel((1, 0, 0), name_suffix='con X'),\n        chronos_cov1",
            "phi=0.6, beta=0.2, sigma_y=1.0, sigma_x=1.0, rho_x=0.95",
            SARIMAX_NAME_1,
        ),
    ),
    (
        ...
        exp_uni_cell(
            ...
            "ARIMAX_DGP", "seed=SEED",
            "SARIMAXModel((1, 0, 0), name_suffix='con X'),\n        chronos_cov1",
            "phi=0.6, beta=0.2, sigma_y=1.0, sigma_x=1.0, rho_x=0.0",
            SARIMAX_NAME_1,
        ),
    ),
]

# ── C-B: dinamicas AR extra (casi RW y AR debil) ─────────────────────────────
CB += [
    (
        ...
        exp_uni_cell(
            ...
            "ARIMAX_DGP", "seed=SEED",
            "SARIMAXModel((1, 0, 0), name_suffix='con X'),\n        chronos_cov1",
            "phi=0.99, beta=0.5, sigma_y=1.0, sigma_x=1.0, rho_x=0.7",
            SARIMAX_NAME_1,
        ),
    ),
    (
        ...
        exp_uni_cell(
            ...
            "ARIMAX_DGP", "seed=SEED",
            "SARIMAXModel((1, 0, 0), name_suffix='con X'),\n        chronos_cov1",
            "phi=0.3, beta=0.5, sigma_y=1.0, sigma_x=1.0, rho_x=0.7",
            SARIMAX_NAME_1,
        ),
    ),
]

# ── C-C: configuraciones extra de 2 covariables ──────────────────────────────
CC += [
    (
        ...
        exp_uni_cell(
            ...
            "ARIMAX2Cov_DGP", "seed=SEED",
            "SARIMAXModel((1, 0, 0), name_suffix='con X'),\n        chronos_cov2",
            "phi=0.6, beta1=0.8, beta2=-0.5, sigma_y=1.0, sigma_x=1.0, rho_x=0.7",
            SARIMAX_NAME_1,
        ),
    ),
    (
        ...
        exp_uni_cell(
            ...
            "ARIMAX2Cov_DGP", "seed=SEED",
            "SARIMAXModel((1, 0, 0), name_suffix='con X'),\n        chronos_cov2",
            "phi=0.6, beta1=0.3, beta2=0.3, sigma_y=1.0, sigma_x=1.0, rho_x=0.7",
            SARIMAX_NAME_1,
        ),
    ),
]

# ── C-D: canal de varianza fuerte y GARCH persistente ────────────────────────
CD += [
    (
        "### C-D.4 — ARIMAX-GARCH: canal de varianza fuerte (delta_var = 0.5)\n\n"
        "**DGP:** media con $\\beta_{\\text{mean}} = 0.5$ y varianza con $\\delta_{\\text{var}} = 0.5$ "
        "($X_t^2$ inyectado fuerte).\n\n"
        ...
        exp_uni_cell(
            ...
            "SARIMAXModel((1, 0, 0), name_suffix='con X'),\n        chronos_cov1",
            "phi=0.4, beta_mean=0.5, omega=0.1, alpha=0.1, beta_garch=0.75, "
            "delta_var=0.5, sigma_x=1.0, rho_x=0.7",
            SARIMAX_NAME_1,
            checks_var="CHECKS_ARIMAX_GARCH",
        ),
    ),
    (
        ...
        "**DGP:** misma media ($\\beta_{\\text{mean}} = 0.5$, $\\delta_{\\text{var}} = 0.1$) con GARCH mas "
        "inercial ($\\alpha = 0.2$, $\\beta_{\\text{garch}} = 0.7$).\n\n"
        ...
        exp_uni_cell(
            ...
            "ARIMAX_GARCH_DGP", "seed=SEED",
            "SARIMAXModel((1, 0, 0), name_suffix='con X'),\n        chronos_cov1",
            "phi=0.4, beta_mean=0.5, omega=0.1, alpha=0.2, beta_garch=0.7, "
            "delta_var=0.1, sigma_x=1.0, rho_x=0.7",
            SARIMAX_NAME_1,
            checks_var="CHECKS_ARIMAX_GARCH",
        ),
...
# ── C-E: causalidad, Sigma negativa y sistema casi-explosivo ─────────────────
CE += [
    (
        "### C-E.4 — VARX con causalidad unidireccional (Y1 -> Y2)\n\n"
        "**DGP:** $A = \\begin{pmatrix} 0.5 & 0.0 \\\\ 0.4 & 0.5 \\end{pmatrix}$ (triangular inferior), "
        "$\\boldsymbol{\\gamma} = (0.5, 0.3)$.\n\n"
        ...
        exp_mv_cell(
            ...
            "seed=SEED, A=[[0.5, 0.0], [0.4, 0.5]], gamma=[0.5, 0.3], "
            "Sigma=[[1.0, 0.3], [0.3, 1.0]], sigma_x=1.0, rho_x=0.7",
            "VARMAXModel(1),\n        chronos_mv_cov1",
            "",
            VARMAX_NAME_1,
    ...
    (
        ...
        "**DGP:** $A = \\begin{pmatrix} 0.5 & 0.1 \\\\ 0.1 & 0.5 \\end{pmatrix}$, "
        "$\\Sigma = \\begin{pmatrix} 1.0 & -0.5 \\\\ -0.5 & 1.0 \\end{pmatrix}$, $\\boldsymbol{\\gamma} = (0.5, 0.3)$.\n\n"
        ...
        exp_mv_cell(
            ...
            "seed=SEED, A=[[0.5, 0.1], [0.1, 0.5]], gamma=[0.5, 0.3], "
            "Sigma=[[1.0, -0.5], [-0.5, 1.0]], sigma_x=1.0, rho_x=0.7",
            "VARMAXModel(1),\n        chronos_mv_cov1",
            "",
            VARMAX_NAME_1,
        ),
    ...
    (
        ...
        "**DGP:** $A = \\begin{pmatrix} 0.7 & 0.2 \\\\ 0.2 & 0.7 \\end{pmatrix}$ "
        "($\\max|\\lambda| = 0.9$), $\\boldsymbol{\\gamma} = (0.5, 0.3)$.\n\n"
        ...
        exp_mv_cell(
            ...
            "seed=SEED, A=[[0.7, 0.2], [0.2, 0.7]], gamma=[0.5, 0.3], "
            "Sigma=[[1.0, 0.3], [0.3, 1.0]], sigma_x=1.0, rho_x=0.7",
            "VARMAXModel(1),\n        chronos_mv_cov1",
            "",
            VARMAX_NAME_1,
...
# ── C-F: velocidades de correccion extra (celdas manuales 3-modelos) ─────────
CF += [
    (
        "### C-F.3 — Cointegracion ADL-ECM, correccion lenta (alpha_ecm = -0.1)\n\n"
        "**DGP:** $\\Delta Y_t = -0.1\\,(Y_{t-1} - X_{t-1}) + \\Delta X_t + \\eta_t$, "
        "$X_t = X_{t-1} + u_t$ (random walk).\n\n"
...
# ── C-G: factorial delta x beta completa + tendencia muy fuerte ──────────────
CG += [
    (
        ...
        exp_uni_cell(
            ...
            "ARIMAX_TREND_DGP", "seed=SEED",
            "SARIMAXModel((1, 0, 0), trend='ct', name_suffix='con X+trend'),\n        chronos_cov1",
            "phi=0.6, beta=0.8, alpha=0.0, delta=0.10, sigma_y=1.0, sigma_x=1.0, rho_x=0.7",
            SARIMAX_NAME_TREND,
            checks_var="CHECKS_ARIMAX_TREND",
        ),
    ),
    (
        ...
        exp_uni_cell(
            ...
            "ARIMAX_TREND_DGP", "seed=SEED",
            "SARIMAXModel((1, 0, 0), trend='ct', name_suffix='con X+trend'),\n        chronos_cov1",
            "phi=0.6, beta=0.5, alpha=0.0, delta=0.20, sigma_y=1.0, sigma_x=1.0, rho_x=0.7",
            SARIMAX_NAME_TREND,
            checks_var="CHECKS_ARIMAX_TREND",
        ),
    ),
...
# ── C-H: AR estacional debil (Phi = 0.4) para s=4 y s=12 ─────────────────────
CH += [
    (
        ...
        exp_uni_cell(
            ...
            "SARIMAX_SEASONAL_DGP", "seed=SEED",
            "SARIMAXModel((1, 0, 0), seasonal_order=(1, 0, 0, 4), name_suffix='con X'),\n        chronos_cov1",
            "s=4, phi=0.3, Phi=0.4, beta=0.5, sigma_y=1.0, sigma_x=1.0, rho_x=0.7",
            SARIMAX_NAME_SEAS_4,
            checks_var="CHECKS_ARIMAX_SEASONAL",
        ),
    ),
    (
        ...
        exp_uni_cell(
            ...
            "SARIMAX_SEASONAL_DGP", "seed=SEED",
            "SARIMAXModel((1, 0, 0), seasonal_order=(1, 0, 0, 12), name_suffix='con X'),\n        chronos_cov1",
            "s=12, phi=0.3, Phi=0.4, beta=0.5, sigma_y=1.0, sigma_x=1.0, rho_x=0.7",
            SARIMAX_NAME_SEAS_12,
            checks_var="CHECKS_ARIMAX_SEASONAL",
        ),
    ),
...
# ── C-I (NUEVO) — Relacion senal/ruido (variacion de sigma_y, sigma_x) ───────
CI = [
    (
        ...
        exp_uni_cell(
            ...
            "ARIMAX_DGP", "seed=SEED",
            "SARIMAXModel((1, 0, 0), name_suffix='con X'),\n        chronos_cov1",
            "phi=0.6, beta=0.5, sigma_y=2.0, sigma_x=1.0, rho_x=0.7",
            SARIMAX_NAME_1,
        ),
    ),
    (
        ...
        exp_uni_cell(
            ...
            "ARIMAX_DGP", "seed=SEED",
            "SARIMAXModel((1, 0, 0), name_suffix='con X'),\n        chronos_cov1",
            "phi=0.6, beta=0.5, sigma_y=0.5, sigma_x=1.0, rho_x=0.7",
            SARIMAX_NAME_1,
        ),
    ),
    (
        ...
        exp_uni_cell(
            ...
            "ARIMAX_DGP", "seed=SEED",
            "SARIMAXModel((1, 0, 0), name_suffix='con X'),\n        chronos_cov1",
            "phi=0.6, beta=0.5, sigma_y=1.0, sigma_x=2.0, rho_x=0.7",
            SARIMAX_NAME_1,
        ),
    ),
    (
        ...
        exp_uni_cell(
            ...
            "ARIMAX_DGP", "seed=SEED",
            "SARIMAXModel((1, 0, 0), name_suffix='con X'),\n        chronos_cov1",
            "phi=0.6, beta=0.5, sigma_y=1.0, sigma_x=0.5, rho_x=0.7",
            SARIMAX_NAME_1,
        ),
    ),
]

# ── C-J (NUEVO) — Interaccion dinamica AR x persistencia de X (phi x rho_x) ──
CJ = [
    (
        ...
        exp_uni_cell(
            ...
            "ARIMAX_DGP", "seed=SEED",
            "SARIMAXModel((1, 0, 0), name_suffix='con X'),\n        chronos_cov1",
            "phi=0.3, beta=0.5, sigma_y=1.0, sigma_x=1.0, rho_x=0.95",
            SARIMAX_NAME_1,
        ),
    ),
    (
        ...
        exp_uni_cell(
            ...
            "ARIMAX_DGP", "seed=SEED",
            "SARIMAXModel((1, 0, 0), name_suffix='con X'),\n        chronos_cov1",
            "phi=0.9, beta=0.5, sigma_y=1.0, sigma_x=1.0, rho_x=0.95",
            SARIMAX_NAME_1,
        ),
    ),
    (
        ...
        exp_uni_cell(
            ...
            "ARIMAX_DGP", "seed=SEED",
            "SARIMAXModel((1, 0, 0), name_suffix='con X'),\n        chronos_cov1",
            "phi=0.9, beta=0.5, sigma_y=1.0, sigma_x=1.0, rho_x=0.0",
            SARIMAX_NAME_1,
        ),
    ),
]
...
# Build cells list
# ────────────────────────────────────────────────────────────────────────────
# Cada bloque es (header_markdown, lista_de_experimentos). Agregar/quitar
# experimentos solo requiere editar la lista del bloque: el ensamblaje recorre
# las listas por nombre, sin indices hardcodeados.
BLOCKS = [
    (BLOCK_CA, CA),
    (BLOCK_CB, CB),
    (BLOCK_CC, CC),
...
for header_md, block_exps in BLOCKS:
    cells.append(md(header_md))
    for intro_md, code_src in block_exps:
...
n_exp = sum(len(block_exps) for _, block_exps in BLOCKS)
print(f"Wrote {NB_PATH}")
print(f"Total cells: {len(cells)}")
print(f"Total experiments: {n_exp}")
print("Por bloque: " + ", ".join(
    f"{hdr.splitlines()[1].split('—')[0].replace('## Bloque', '').strip()}={len(exps)}"
    for hdr, exps in BLOCKS
))
